...
export default function Admin() {
  ...
  function addGalleryImage(event) {
    ...
    if (!url) {
      setStatus("Paste one image URL before clicking Add image.");
      return;
    }
    if (availableSlots <= 0) {
    ...
    if ((form.images || []).includes(url)) {
      setStatus("That gallery image has already been added.");
      return;
    }
    setForm((current) => ({
      ...current,
      images: [...(current.images || []), url].slice(0, 3),
    }));
    setGalleryUrl("");
    setStatus("Gallery image added. Add another URL or save the project.");
  }
